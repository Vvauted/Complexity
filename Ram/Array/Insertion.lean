...
Authors: vvauted
-/
import Ram.Array.Insertion.Contents
...
private structure Invariant (heapLimit : Nat) (base : Word w) (position : Nat)
  ...
  array : ArrayAt heapLimit base (contents xs spare (s.regs 4).toNat) s
...
private theorem step_preserves {heapLimit position : Nat} {base key spare : Word w}
  ...
  have hlen := contents_length xs spare hupper
  have hc : c < (contents xs spare c).length := by rw [hlen]; omega
  have hpred : c - 1 < (contents xs spare c).length := by rw [hlen]; omega
  ...
  have hload : s.mem (s.regs 0 + (s.regs 4 - 1)) = xs[c - 1] := by
    ...
    exact (h.array.1.lookup (c - 1) hpred).trans
      (contents_getElem_pred xs spare hpos hupper)
  ...
  have hframe : ArrayFrame base (xs.length + 1) s.mem (stepResult s).mem := by
    rw [hmem]
    have hf : ArrayFrame base (contents xs spare c).length s.mem stored.mem :=
      ArrayFrame.store h.array.1 hc xs[c - 1]
    rw [hlen] at hf
    exact hf
  have harray : ArrayAt heapLimit base (contents xs spare (c - 1)) (stepResult s) := by
    have ha := h.array.setMem hc xs[c - 1]
    rw [contents_shift xs spare hpos hupper] at ha
...
private theorem finish_post {heapLimit position : Nat} {base key spare : Word w}
  ...
  have ha : ArrayAt heapLimit base (contents xs spare position) s := by
    simpa only [hc] using h.array
  have hlen := contents_length xs spare hp
  have hindex : position < (contents xs spare position).length := by rw [hlen]; omega
  ...
  · have hs := ha.setMem hindex key
    rw [contents_set_eq_insertIdx xs spare key hp] at hs
    exact hs
  · have hf : ArrayFrame base (contents xs spare position).length s.mem
...
theorem contract {control heapLimit depth : Nat} {functions : Program}
  ...
  have hloop : Contract control functions heapLimit depth loop
    ...
    · intro s hs hz
      ...
      have hlen := contents_length xs spare hs.upper
  ...
  have hsafe : (sl.regs 0 + sl.regs 2).toNat < heapLimit := by
    ...
    rw [contents_length xs spare hsl.1.upper]
