...
def comp (q : Component B C g E) (p : Component A B f D)
  ...
  timeBound n := p.timeBound n + monotoneHull q.timeBound (p.sizeBound n)
  heapBound n := max (p.heapBound n) (monotoneHull q.heapBound (p.sizeBound n))
  depthBound n := max (p.depthBound n) (monotoneHull q.depthBound (p.sizeBound n))
  sizeBound n := monotoneHull q.sizeBound (p.sizeBound n)
  size_le w x hx := Nat.le_trans (q.size_le w (f x) (hdom w x hx))
    (Nat.le_trans (le_monotoneHull q.sizeBound _) (hull_monotone _ (p.size_le w x hx)))
  correct x hx s hs H d hH hd := by
    ...
    have hhq : q.heapBound (B.size (f x)) ≤ H :=
      Nat.le_trans (le_monotoneHull _ _) (Nat.le_trans (hull_monotone _ hsize)
...
@[simp] theorem comp_heapBound (q : Component B C g E) (p : Component A B f D)
    ...
    (q.comp p hdom).heapBound n =
      max (p.heapBound n) (monotoneHull q.heapBound (p.sizeBound n)) := rfl
...
@[simp] theorem comp_depthBound (q : Component B C g E) (p : Component A B f D)
    ...
    (q.comp p hdom).depthBound n =
      max (p.depthBound n) (monotoneHull q.depthBound (p.sizeBound n)) := rfl
...
theorem comp_totalTime (q : Component B C g E) (p : Component A B f D)
    ...
    (q.comp p hdom).totalTime n =
      p.timeBound n + monotoneHull q.timeBound (p.sizeBound n) + 2 := rfl

theorem totalTime_polynomial (p : Component A B f D)
    (ht : IsPolynomiallyBounded p.timeBound) : IsPolynomiallyBounded p.totalTime :=
  ht.add (IsPolynomiallyBounded.const 2)
...
theorem comp_time_polynomial (q : Component B C g E) (p : Component A B f D)
    ...
    (hp : IsPolynomiallyBounded p.timeBound) (hq : IsPolynomiallyBounded q.timeBound)
    (hs : IsPolynomiallyBounded p.sizeBound) :
    IsPolynomiallyBounded (q.comp p hdom).timeBound := hp.add (hq.monotoneHull.comp hs)

theorem comp_size_polynomial (q : Component B C g E) (p : Component A B f D)
    (hdom : ∀ w x, D w x → E w (f x))
    (hp : IsPolynomiallyBounded p.sizeBound) (hq : IsPolynomiallyBounded q.sizeBound) :
    IsPolynomiallyBounded (q.comp p hdom).sizeBound := hq.monotoneHull.comp hp
...
theorem comp_heap_polynomial (q : Component B C g E) (p : Component A B f D)
    (hdom : ∀ w x, D w x → E w (f x))
    (hp : IsPolynomiallyBounded p.heapBound) (hq : IsPolynomiallyBounded q.heapBound)
    (hs : IsPolynomiallyBounded p.sizeBound) :
    IsPolynomiallyBounded (q.comp p hdom).heapBound := hp.max (hq.monotoneHull.comp hs)

theorem comp_depth_polynomial (q : Component B C g E) (p : Component A B f D)
    (hdom : ∀ w x, D w x → E w (f x))
    (hp : IsPolynomiallyBounded p.depthBound) (hq : IsPolynomiallyBounded q.depthBound)
    (hs : IsPolynomiallyBounded p.sizeBound) :
    IsPolynomiallyBounded (q.comp p hdom).depthBound := hp.max (hq.monotoneHull.comp hs)
...
structure PolyTimeComponent {α : Type u} {β : Type v}
  ...
  time_polynomial : IsPolynomiallyBounded timeBound
  /-- A polynomial bound on the mathematical result size. -/
  size_polynomial : IsPolynomiallyBounded sizeBound
...
def id (A : Interface α) (domain : Nat → α → Prop) :
  ...
  time_polynomial := IsPolynomiallyBounded.const 0
  size_polynomial := IsPolynomiallyBounded.id
