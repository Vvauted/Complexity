...
contracts are in `Complexity.Computability.Ram.Array.Contracts`. Swap permits equal indices
but records its actual temporary-register freshness requirement.
...
[the backend chapter](##ComplexityDocs.Backend).
