...
Read [functional verification](##ComplexityDocs.Verification) next for the proof workflow.
...
All reusable imports start with `Complexity`. For example:
...
The first module provides RAM refinement interfaces; the second provides arithmetic
recurrence lemmas without importing a RAM model. Declaration namespaces follow the objects:
`Ram.Source.Refines` is a RAM relation, whereas `Recurrence.le_of_div_le` is numerical.
The package root does not become a prefix on every declaration. Examples have the separate
entry point `Examples`.

## Programs and named variables

The frontend in `Complexity.Computability.Ram.Source.Syntax` resolves named locals to
registers. The source vocabulary includes expressions, array-addressed reads and writes,
assignment, input and output, conditionals, loops, first-order calls and recursion.
...
Import `Complexity.Computability.Ram.Source.Named.Declaration` (or `Complexity`) to use
the declaration form `ram_def p := ram_program% { ... }` when proofs need source names.
...
| --- | --- |
| `Examples.Ram.Verification` | I/O refinement, native `StateM` and separate time analysis |
| `Examples.Ram.Composition` | Linking independent modules through a shared representation |
| `Examples.Ram.ArraySum` | A native traversal model connected to a fixed array loop |
| `Examples.Ram.Factorial` | Mathematical recursive specification using `Nat.factorial` |
| `Examples.Ram.BitLength` | A well-founded numerical model and a separate logarithmic time bound |
| `Examples.Ram.AmortizedClear` | Potential-based analysis of actual push and clear operations |
| `Examples.Ram.ArrayCopy` | Reusing a preloaded-array contract and exporting its memory frame |
| `Examples.Ram.BinarySearch` | Lower bound, duplicates and ordinary `List.findIdx` specifications |
| `Examples.Ram.MergeSort` | Recursion, borrowed arrays, scratch and a balanced recurrence |

The library's `Complexity.Computability.Ram.Matrix.MergeSort` is a reusable implementation
that connects the sorting call to a native matrix-row model.

`Examples.Ram.Arithmetic` distinguishes modular and no-overflow multiplication. Array fill,
...
Compile the examples separately with `lake build Examples`. To run the existing native
demonstration:
