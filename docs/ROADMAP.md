...
- [ ] Compose mathematical functions and state contracts through the same
  ...
  Default scalar branches also need the exact unchanged-heap equations and
  curried total contracts previously generated by the pure compatibility path.
  A result refinement or preservation of old array contents does not imply
  equality of the complete final heap. Keep this obligation in the shared
  generator, instead of replacing short existing user proofs with manual
  monadic branch proofs.
