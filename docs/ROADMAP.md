...
`if` and Option/List `match` value bindings share the state-carrying proof boundary, retaining
final outer mutable bindings across their existing joins. Their correspondence
is heap-indexed, not an unchanged-heap exact equation. The selected branch and
original return-marker placement are preserved; this does not complete general
nested-loop or effectful-control integration.
