...
/-!
# Arithmetic bounds for recurrences

These comparison lemmas bound natural-valued functions satisfying successor or
dividing recurrences. They are independent of a programming language or machine.
To use them for complexity analysis, separately justify that the chosen recurrence
accounts for all work performed by the implementation.
...
theorem le_geometric_of_div_le {T : Nat → Nat} {divisor branches initial toll : Nat}
  ...
  intro m hm
  rw [← Nat.clog_div_succ_add_one divides hm, geometricBudget_succ]
