...
def budget (n : Nat) : Nat := Recurrence.balancedBudget 4 158 n
...
theorem depth_left {n : Nat} (hn : 2 ≤ n) : depth (n / 2) + 1 ≤ depth n := by
  ...
  exact (Nat.add_le_add_right hm 1).trans_eq (Recurrence.clog_ceil_half_add_one hn)

theorem depth_right {n : Nat} (hn : 2 ≤ n) : depth (n - n / 2) + 1 ≤ depth n :=
  Nat.le_of_eq (Recurrence.clog_ceil_half_add_one hn)
...
theorem budget_split_cover {n : Nat} (hn : 2 ≤ n) :
  ...
  exact (Nat.add_le_add_left (by omega : 53 * n + 209 ≤ 158 * n) _).trans
    (Recurrence.balancedBudget_split_le 4 158 hn)
...
theorem budget_isBigO :
  ...
  Recurrence.balancedBudget_isBigO 4 158
...
theorem block_budget_le (n : Nat) :
    budget n + 82 ≤ Recurrence.balancedBudget 86 158 n :=
  Recurrence.balancedBudget_add_const_le 4 158 82 n
...
theorem block_budget_isBigO :
  ...
  refine (Asymptotics.IsBigO.of_norm_le (g := fun n =>
    (Recurrence.balancedBudget 86 158 n : ℝ)) ?_).trans
      (Recurrence.balancedBudget_isBigO 86 158)
  intro n
  simpa only [Real.norm_natCast] using (Nat.cast_le.mpr (block_budget_le n) :
    ((budget n + 82 : Nat) : ℝ) ≤ (Recurrence.balancedBudget 86 158 n : ℝ))
