...
theorem function_timeBound_of_depth {w control heapLimit depth : Nat}
  ...
  | h xs ih =>
    ...
    · ram_time_start args entry ⟨rfl, sourceArray, scratchArray⟩
      ram_time_branch sortFunctions.function.sort else at initial with bindings
        [BitVec.toNat_one positive, sourceArray.1,
          show ¬1 < xs.length by omega, bodyBudget_small small]
    · have large : 2 ≤ xs.length := by omega
      ...
      ram_time_start args entry ⟨rfl, sourceArray, scratchArray⟩
      ram_time_branch sortFunctions.function.sort then at initial with bindings
        [BitVec.toNat_one positive, sourceArray.1,
      ...
      ram_time_apply leftCorrect leftTime on
        (initial.left, initial.scratch.subslice 0 k) [ArrayRef.subslice, k]
      · exact ⟨by simp [ArrayRef.subslice, initial.left, initial.scratch,
          k, initial.middle], leftSource, leftScratch⟩
      · exact ⟨by simp [ArrayRef.subslice, initial.left, initial.scratch,
          k, initial.middle], leftSource, leftScratch⟩
      ...
      · rintro _ afterLeft ⟨leftSorted, _, leftFrame, _, _⟩
        ...
        ram_time_apply rightCorrect rightTime on
          ...
          [ArrayRef.subslice, k]
        · exact ⟨by simp [ArrayRef.subslice, initial.right, initial.scratch,
            initial.xs, k, initial.middle], rightSource, rightScratch⟩
        · exact ⟨by simp [ArrayRef.subslice, initial.right, initial.scratch,
            initial.xs, k, initial.middle], rightSource, rightScratch⟩
        · ram_bound [sortFunctions.result_eq.sort, List.length_take, List.length_drop,
            min_eq_left contained]
        · rintro _ afterRight ⟨rightSorted, _, rightFrame, _, _⟩
          ...
          ram_time_apply mergeCorrect mergeTime on
            ...
            [ArrayRef.subslice, k]
          · exact ⟨by simp [ArrayRef.subslice, initial.left, initial.right,
              initial.scratch, k, initial.middle], mergeLeft, mergeRight, scratchAfterRight⟩
          · exact ⟨by simp [ArrayRef.subslice, initial.left, initial.right,
              initial.scratch, k, initial.middle], mergeLeft, mergeRight, scratchAfterRight⟩
          ...
          · rintro _ afterMerge ⟨_, _, scratchMerged, mergeFrame, _, _⟩
            ...
            ram_time_call copyTime
              [bindings.scratch.base, bindings.xs.base, bindings.xs.length,
                initial.scratch, initial.xs, ArrayRef.subslice, sourceArray.length_eq, length_sorted,
