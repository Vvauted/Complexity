...
returned value and shared state. It requires neither a `main` program nor a
...
theorem FunctionExec.mono_heap {heapLimit heapLimit' depth : Nat} {program : Program}
    {f : Func} {args : List (Word w)} {entry finish : State w} {value : Word w}
  ...
  exact ⟨arity, frame, callee, body.mono_heap hh, result.mono_heap hh, returned, shared⟩

/-- Total correctness of a function, stated on arguments, return value and
...
def FunctionContract (program : Program) (heapLimit depth : Nat) (f : Func)
    ...
    (Q : List (Word w) → State w → Word w → State w → Prop) : Prop :=
...
theorem of_body {R : State w → Prop} {S : State w → State w → Prop}
    (body : TotalRelContract program heapLimit depth f.body R
      (fun entered callee => f.result.ReadsBelow heapLimit callee.regs callee.mem ∧
        S entered callee))
    ...
    (post : ∀ args entry, P args entry → ∀ callee, S (entry.enter args) callee →
      Q args entry (callee.eval f.result) (entry.restore callee)) :
    FunctionContract program heapLimit depth f P Q := by
  intro args entry hp
...
The continuation sees the returned value, actual shared effects and restored
caller locals. Local preservation need not be added to the user postcondition. -/
theorem wp_call {fn dst callDepth : Nat} {exprs : List Expr} {entry : State w}
    ...
    (continuation : ∀ value finish, Q (exprs.map entry.eval) entry value finish →
      finish.regs = entry.regs → post (finish.setReg dst value)) :
    Verification.TotalWP program heapLimit callDepth (.call dst fn exprs) post entry := by
  obtain ⟨value, finish, execution, result⟩ := h _ entry pre
  exact ⟨_, (execution.call lookup arguments).mono nesting,
