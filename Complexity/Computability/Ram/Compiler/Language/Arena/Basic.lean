...
cursor. Every represented object cell is positive and below that cursor;
...
/-- Transport heap representation using equality only on actual object cells.
...
theorem of_mem_eq_on {placement : Nat → Word w} {heapLimit : Nat}
    ...
    (same : ∀ {τ : CellTy} {object : Nat} {values : Array (CellValue τ)},
      heap.object? τ object = some values → ∀ index, index < values.size →
        finish.mem (arrayAddr (placement object) index) =
          entry.mem (arrayAddr (placement object) index)) :
  ...
  exact (same found index (by
    simpa only [objectWords, Array.length_toList, Array.size_map] using bound)).trans
...
theorem of_mem_eq_on (arena : ArenaRep placement next heapLimit heap entry)
  ...
  · intro τ object values found index bound
    exact same _ (arena.reserved found index bound).1 (arena.reserved found index bound).2
  · intro τ object values found index bound
    exact ⟨(arena.reserved found index bound).1,
      lt_of_lt_of_le (arena.reserved found index bound).2 grows⟩
...
theorem heapRep_setMem_fresh (arena : ArenaRep placement next heapLimit heap entry)
  ...
  intro τ object values found index bound
  simp only [Source.State.setMem, if_neg (arena.address_ne_fresh found bound fresh)]
