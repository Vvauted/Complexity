...
`realizable` accepts any heap-indexed accumulator representation, with the
existing read-only condition on the callback program. `costBound` is conditional
...
theorem realizable
  ...
  refine FunctionRealizable.of_arenaResources (post := fun _ _ _ _ => True)
    (functionResources correct resources bounded positive) ?_
...
theorem costBound
  ...
  apply FunctionCostBound.of_arenaCostBound
    (functionResources correct resources bounded positive)
