...
cursor and its stronger remaining-capacity premise, without a callback time
bound or another traversal termination proof. The cost contract reuses
...
/-- Establish readiness of the actual fold entry from callback resources and
the independent source contract. Word ranges and remaining capacity are
required, but no callback time bound or second termination argument is used. -/
theorem ready
  ...
  have executed := body_ready
    (Complexity.Language.List.Fold.callee_contract sourceProgram fn same correct)
    linkedResources mathematical values accumulator root heap cursor positive
    allowed related accFits headFits capacity observed
  rw [← Complexity.Language.List.Fold.program_body sourceProgram fn same] at executed
  exact executed
...
theorem measured
  ...
  exact ⟨finalAcc, finalHeap, finalCursor, steps, execution, ready, cost,
    functionCostBound_of_actual correct bounded (mathematical, values, accumulator, root) heap
      ⟨allowed, related, observed⟩ _ _ execution ready cost, cursorBound⟩

/-- The actual callable fold's resource certificate requires no proposed time
bound. Its reservation and readiness concern the same supplied execution. -/
theorem functionResources_of_ready
    {sourceProgram : Complexity.Language.Program signatures} {fn : Fin signatures.length}
    {same : signatures[fn] = Complexity.Language.List.Fold.stepSignature accTy kind}
    {representation : Representation α accTy} {step : α → CellValue kind → α}
    {domain : α → CellValue kind → Prop}
  ...
  obtain ⟨finalAcc, finalHeap, finalCursor, actual, ready, cursorBound⟩ :=
    ready correct resources mathematical values accumulator root heap cursor
      positive allowed related accFits headFits capacity observed
  obtain ⟨rfl, sameControl⟩ := actual.deterministic execution
...
theorem functionResources
    ...
    FunctionArenaResources (Complexity.Language.List.Fold.program sourceProgram fn same)
      ...
      w heapLimit (depth + 1) (fun input => accumulated step reserve input.1 input.2.1) := by
  clear bounded
  exact functionResources_of_ready correct resources positive
