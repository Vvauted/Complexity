...
The following proof is taken from `Ram.Examples.Verification`. There, `main` reads a word,
...
For example, `Ram.Examples.Verification` contains:
...
[data models and memory](##Complexity.Doc.Models).
