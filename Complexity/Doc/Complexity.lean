...
For pure arithmetic on a finite history, `Ram.CostSum.sum_range_add_potential_le` derives:
...
definition requiring a separate equivalence proof. `Ram.IsPolynomiallyBounded f` asserts
...
Moving sums need uniform bounds. `CostSum.isBigO_sum_of_uniform` takes one coefficient and
...
`Ram.Recurrence` analyzes already justified numerical bounds. A recurrence must include
...
The finite-prefix maximum `monotoneHull` permits substitution into a nonmonotone bound and
