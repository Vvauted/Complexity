...
For implementation details, see `Ram.LocalABI`, `Ram.LocalCallSetup`, `Ram.LocalCallReturn`,
`Ram.LocalCall`, `Ram.LocalMeasured` and `Ram.LocalProgram`.
...
Complete recursive simulation in `Ram.LocalMeasured.Memory` bounds accesses using the real
entry SP and remaining nesting envelope. `Ram.LocalMeasured.Writes` separates source-heap
...
These lemmas are in `Ram.LocalABI.Slots`, `Ram.LocalABI.Lifetime`, `Ram.LocalABI.Stack` and
`Ram.LocalExact.Stack`. They describe the real protocol's obligations, not a heap allocator
