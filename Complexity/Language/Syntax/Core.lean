...
private def scopeCompletionDeclarations (program : TSyntax `ident) (site : BlockSite)
    (sites : Array BlockSite) : MacroM (Array Syntax) := do
  let some target := site.localReturn | return #[]
  if site.guard.isSome then return #[]
...
private def programDeclarations (family : TSyntax `ident)
  ...
  for site in loopSites do
    ...
    declarations := declarations ++ (← scopeCompletionDeclarations programName site loopSites)
